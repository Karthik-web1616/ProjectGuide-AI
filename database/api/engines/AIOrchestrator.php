...
class AIOrchestrator {
    ...
    private function checkOllama() {
        ...
        $res = $this->httpRequest(OLLAMA_URL . '/api/tags', 'GET', null, 5);
        $code = $res['code'];
        $response = $res['body'];
        $error = $res['error'];
    ...
    public function chat($message, $history = [], $language = 'en', $context = []) {
        ...
        $latencyMs = microtime(true) * 1000 - $startMs;

        if ($reply === null) {
            $this->sendTrace('chat', $message, '', $reply ?? '', $latencyMs, 'error');
            return [
                'reply'      => null,
        ...
        $this->sendTrace('chat', $message, $systemPrompt, $reply, $latencyMs);
    ...
    public function analyzePlant($imageBase64, $message = '', $language = 'en', $context = []) {
        ...
        $latencyMs = microtime(true) * 1000 - $startMs;
        $this->sendTrace('vision', $message ?: 'image analysis', '', $reply, $latencyMs);
    ...
    public function generateBulletin($weatherData, $locationName, $language = 'en', $context = []) {
        ...
        $latencyMs = microtime(true) * 1000 - $startMs;
        $this->sendTrace('bulletin', "Location:{$locationName} {$weatherData}", '', $reply ?? '', $latencyMs, $reply ? 'ok' : 'error');
    ...
    public function suggestCrops($soil, $season, $locationName, $language = 'en', $context = []) {
        ...
        $latencyMs = microtime(true) * 1000 - $startMs;
        $this->sendTrace('suit', "soil:{$soil} season:{$season} loc:{$locationName}", '', $reply ?? '', $latencyMs, $reply ? 'ok' : 'error');
    ...
    public function explainData($data, $language = 'en', $context = []) {
        ...
        $latencyMs = microtime(true) * 1000 - $startMs;
        $this->sendTrace('explain', $data, '', $reply ?? '', $latencyMs, $reply ? 'ok' : 'error');
    ...
    private function rawOllamaChat($messages, $model, $stream = false, $images = [], $maxTokens = 150, $timeout = 30) {
        ...
        $res = $this->httpRequest(OLLAMA_URL . '/api/chat', 'POST', $data, $timeout);
        $code = $res['code'];
        $response = $res['body'];
        $curlError = $res['error'];
    ...
    // ═══════════════════════════════════════════════════
    // TRACING — fire-and-forget to Trace AI microservice
    // ═══════════════════════════════════════════════════

    /**
     * Send one trace record to the Trace AI microservice.
     * Uses a 1 s connect / 1 s total timeout — if the service is
     * down or slow, this silently fails and the app keeps working.
     */
    private function sendTrace(
        string $action,
        string $query,
        string $systemPrompt,
        string $response,
        ...
        $traceData = [
            'action'        => $action,
            'query'         => substr($query, 0, 2000),       // cap at 2 KB
            'model'         => $this->activeModel ?? 'unknown',
            'response'      => substr($response, 0, 4000),    // cap at 4 KB
            'latency_ms'    => round($latencyMs, 2),
            'system_prompt' => substr($systemPrompt, 0, 1000),
            'retrieved_json'=> '[]',
            'status'        => $status,
        ];

        $this->httpRequest(TRACE_SERVICE_URL . '/trace', 'POST', $traceData, 1);
    }

    /**
     * Universal HTTP Request helper — supports cURL when enabled or native PHP streams
     */
    private function httpRequest(string $url, string $method = 'GET', ?array $data = null, int $timeout = 5): array {
        if (function_exists('curl_init')) {
            ...
            $error = curl_error($ch);
            curl_close($ch);
            return ['code' => (int)$code, 'body' => $body ?: null, 'error' => $error ?: null];
        }
        ...
        $opts = [
            'http' => [
                'method'        => $method,
                'header'        => $headers,
                'content'       => ($method === 'POST' && $data !== null) ? json_encode($data) : null,
                'ignore_errors' => true,
                'timeout'       => $timeout,
            ]
        ];
        ...
        if (!empty($headers[0])) {
            ...
            $code = isset($m[1]) ? (int)$m[1] : ($body !== false ? 200 : 0);
        }
        return ['code' => $code, 'body' => $body ?: null, 'error' => $body === false ? 'Stream request failed' : null];
    }
