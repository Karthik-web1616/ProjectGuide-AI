...
export default function AdvisoryChat() {
  ...
  const quickQs = [
    t('chat.q1'), t('chat.q2'), t('chat.q3'), t('chat.q4'), t('chat.q5'), t('chat.q6'),
    // Market price quick-presets
    "What's the current market price for rice in nearby mandis?",
    "Show 7-day price trend for rice in nearby mandis.",
    "Compare current market prices for rice and maize in nearby mandis.",
    "Which mandi has the highest price for tomatoes today?",
    "List average mandi prices for onions in my state last week.",
    // Agronomy / analytics presets
  ...
  const sendMessage = async (text) => {
    ...
    try {
      ...
      // Attempt RAG retrieval from the active vector server and append to context.
      // The local vector service runs on 8001; the Gemini helper uses 8002, so we
      // must not hardcode the wrong port or the chatbot will silently fall back to
      ...
      try {
        ...
        for (const url of retrieveUrls) {
          try {
            ...
            // Try the next endpoint if this one is not available.
          }
        }

        if (rv && rv.ok) {
          const jr = await rv.json()
          const chunks = jr.retrieved_chunks || []
          if (chunks.length > 0) {
            aiContext = { ...aiContext, rag_retrieved: chunks }
            const frag = chunks.map(c => `${c.topic}: ${c.text.slice(0, 250)}`).join('\n\n')
            aiContext.knowledge_graph = (aiContext.knowledge_graph || '') + '\n\nRetrieved:\n' + frag
          }
        }
      } catch (e) {
