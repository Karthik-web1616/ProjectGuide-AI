...
<<<<<<< HEAD
  ...
  const handleRunTracking = async (proj, pIndex = null) => {
    ...
    try {
      ...
      // Update project with trackingReport
      let currentProjs = email ? Store.getUserProjects(email) : [...projects];
      let targetIdx = pIndex;
      if (targetIdx === null) {
        targetIdx = currentProjs.findIndex(p => p.title === proj.title || (p.id && p.id === proj.id));
  ...
  const handleToggleMilestone = async (milestoneId, completed) => {
    ...
    try {
      const ideaId = selectedProject.id || selectedProject.idea_id || '';
      ...
      if (selectedTrackingReport && selectedTrackingReport.milestones) {
        const updatedMilestones = selectedTrackingReport.milestones.map(m => {
          if (m.id === milestoneId) {
            return {
              ...m,
              completed,
        ...
        const updatedReport = {
          ...
          milestonesDone: res.milestonesDone,
        ...
        const email = user?.email;
        let currentProjs = email ? Store.getUserProjects(email) : [...projects];
        const pIdx = currentProjs.findIndex(p => p.title === selectedProject.title || (p.id && p.id === selectedProject.id));
        if (pIdx !== -1) {
          currentProjs[pIdx] = {
            ...currentProjs[pIdx],
...
<<<<<<< HEAD
                        ...
                        <div
                          ...
                          onClick={() => {
                            if (trackingProjectTitle === proj.title) return;
                            if (proj.trackingReport) {
                              setSelectedTrackingReport(proj.trackingReport);
                              setSelectedProject(proj);
                              setIsTrackingModalOpen(true);
                            } else {
                              handleRunTracking(proj, pIdx);
                            }
                          }}
                        >
                          <div className="agent-card-top">
                            <div className="agent-icon teal">
                              {trackingProjectTitle === proj.title ? <span className="agent-spin">⚙️</span> : '🗺️'}
                            </div>
                            <div className="agent-name">
                              Tracking Agent
                              <div className="agent-subtext">Milestones · Sprints · Live Progress</div>
                            </div>
                            <span className={`agent-status-pill ${
                              trackingProjectTitle === proj.title ? 'running' :
                              proj.trackingReport ? 'done' :
                              (!proj.feasibilityReport || !proj.scopeReport) ? 'locked' : 'ready'
                            }`}>
                              {trackingProjectTitle === proj.title ? 'Running' :
                               proj.trackingReport ? 'Done' :
                          ...
                          <div style={{
                            fontSize: '0.68rem',
                            color: (proj.feasibilityReport && proj.scopeReport && proj.techStackReport) ? 'rgba(16,185,129,0.8)' : 'rgba(255,255,255,0.25)',
                            marginBottom: '0.4rem',
                            display: 'flex', alignItems: 'center', gap: '0.3rem'
                          }}>
                            <span>🔗</span>
                            <span>
                              {proj.feasibilityReport ? '✅' : '⬜'} Feas →&nbsp;
                              {proj.scopeReport ? '✅' : '⬜'} Scope →&nbsp;
                              {proj.techStackReport ? '✅' : '⬜'} Stack → Tracking
                            </span>
                          </div>

                          {proj.trackingReport && (
                            <div className="agent-verdict-text">
                              🗺️ {proj.trackingReport.overallProgress || 0}% Complete ({proj.trackingReport.milestonesDone || 0}/{proj.trackingReport.totalMilestones || 4} Done)
                            </div>
                          )}
                          ...
                          <div className="agent-card-actions" onClick={e => e.stopPropagation()}>
                            {proj.trackingReport ? (
                              <>
                                <button className="agent-btn agent-btn-teal" onClick={() => { setSelectedTrackingReport(proj.trackingReport); setSelectedProject(proj); setIsTrackingModalOpen(true); }}>
                                  📋 View Roadmap
                                </button>
                                <button className="agent-btn agent-btn-secondary" disabled={trackingProjectTitle === proj.title} onClick={() => handleRunTracking(proj, pIdx)}>
                                  {trackingProjectTitle === proj.title ? <><span className="agent-spin">⟳</span> Running</> : '🔄 Re-run'}
                                </button>
                              </>
                            ) : (
                              <button
                                className="agent-btn agent-btn-teal"
                                disabled={trackingProjectTitle === proj.title || !proj.feasibilityReport || !proj.scopeReport}
                                onClick={() => handleRunTracking(proj, pIdx)}
                                title={!proj.feasibilityReport ? 'Run Feasibility Agent first' : !proj.scopeReport ? 'Run Scope Agent first' : ''}
                              >
                                {trackingProjectTitle === proj.title
                                  ? <><span className="agent-spin">⟳</span> Planning…</>
                                  : (!proj.feasibilityReport || !proj.scopeReport)
                                    ? '🔒 Run prev. agents first'
                                    : '▶ Run Agent'}
                              </button>
